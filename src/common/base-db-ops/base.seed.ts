...
export abstract class BaseSeed<TEntity extends ObjectLiteral>
  implements ISeed<TEntity>
{
  ...
  protected async writeDataAsync(data: Partial<TEntity>[]): Promise<void> {
    ...
    const diff = map(
      ...
      (e: unknown) => this.repo.create(e as TEntity),
    );
    if (diff.length > 0) {
  ...
  protected async loadSeedEntityAsync(): Promise<void> {
    const builder = this.queryBuilder
      .select(['id', 'name', 'version', 'timestamp'])
      .from('public.seeds', 's')
      .where('s.name = :name', { name: this.name });
  ...
  protected async updateSeedEntityAsync(): Promise<void> {
    if (this.version > 1 && this.seedEntity) {
      const builder = this.queryBuilder
        .update('public.seeds')
        .set({ version: this.version, timestamp: new Date() })
        .where('name = :name', { name: this.name });
      await builder.execute();
    } else {
      const builder = this.queryBuilder.insert().into('public.seeds').values({
        name: this.name,
  ...
  protected abstract equalityCheck(
    x: Partial<TEntity>,
    y: Partial<TEntity>,
  ): boolean;

  protected abstract createFilter():

...
export class MenuItemService {
  ...
  async getMenuItem(data: IdParamDto): Promise<MenuItemResponseDto> {
    ...
    if (!entity) {
      throw new HttpException(
        `MenuItem with ID ${data.Id} not found`,
  ...
  async createMenuItem(
    ...
    if (existingMenuItem) {
      throw new HttpException(
        'MenuItem with this name already exists',
        HttpStatus.CONFLICT,
      );
    }
  ...
  async updateMenuItem(
    ...
    if (!entity) {
      throw new HttpException(
        `MenuItem with ID ${params.Id} not found`,
        HttpStatus.NOT_FOUND,
      );
    }
    const existingMenuItem = await this.menuItemRepository.findOne({
    ...
    if (!menuItemResponse) {
      throw new HttpException(
        `Failed to retrieve updated menu item with ID ${params.Id}`,

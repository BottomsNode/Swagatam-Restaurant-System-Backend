...
export class OrderItemService {
  ...
  async getOrderItem(data: IdParamDto): Promise<OrderItemResponseDto> {
    ...
    if (!entity) {
      throw new HttpException(
        `OrderItem with ID ${data.Id} not found`,
  ...
  async updateOrderItem(
    ...
    if (!entity) {
      throw new HttpException(
        `OrderItem with ID ${params.Id} not found`,
        HttpStatus.NOT_FOUND,
      );
    }
    ...
    if (!orderItemResponse) {
      throw new HttpException(
        `Failed to retrieve updated order item with ID ${params.Id}`,

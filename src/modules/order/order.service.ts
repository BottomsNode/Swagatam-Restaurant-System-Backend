...
export class OrderService {
  ...
  async updateOrder(
    ...
    if (!orderResponse) {
      throw new HttpException(
        `Failed to retrieve updated order with ID ${params.Id}`,
  ...
  private async createOrderEntity(
    ...
    if (!table) {
      throw new NotFoundException(
        `Table with ID ${createDto.tableId} not found.`,
      );
    }
  ...
  private async validateAndUpdateMenuItems(
    ...
    for (const item of items) {
      ...
      if (!menuItem) {
        throw new NotFoundException(
          `Menu item with ID ${item.menuItem.id} not found.`,
        );
      }
        ...
        void (async () => {
          try {
            await manager.transaction(async (innerManager) => {
              const freshTable = await innerManager.findOne(TableEntity, {
                where: { id: tableId },
              });
              if (freshTable) {
                ...
                this.logger.log(
                  `Table ${freshTable.id} status reset to AVAILABLE`,
                );
              } else {
              ...
              if (freshOrder) {
                ...
                this.logger.log(
                  `Order ${freshOrder.id} status updated to COMPLETED`,
                );
              } else {
                this.logger.error(
                  `Order ${orderId} not found during status update`,
                );
              }
            });
          } catch (resetError) {
            if (resetError instanceof Error) {
              this.logger.error(
                `Error during scheduled status reset: ${resetError.message}`,
              );
            }
          }
        })();

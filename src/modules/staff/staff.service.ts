...
export class StaffService {
    ...
    async createStaff(createDto: CreateStaffDto): Promise<StaffResponseDto> {
        ...
        if (existingStaff) {
            throw new HttpException('Staff with this email already exists', HttpStatus.CONFLICT);
        }
    ...
    async updateStaff(params: IdParamDto, updateDto: CreateStaffDto): Promise<StaffResponseDto> {
        const entity = await this.staffRepository.findOne({
            where: { id: params.Id },
        });
        if (!entity) {
            throw new HttpException(`Staff with ID ${params.Id} not found`, HttpStatus.NOT_FOUND);
        }
        const existingStaff = await this.staffRepository.findOne({
        ...
        const updatedEntity = this.mapper.map(updateDto, CreateStaffDto, StaffEntity);
        ...
        if (!staffResponse) {
            throw new HttpException(`Failed to retrieve updated staff with ID ${params.Id}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }

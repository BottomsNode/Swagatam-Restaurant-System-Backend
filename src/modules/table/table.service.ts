...
export class TableService {
  ...
  async getTable(data: IdParamDto): Promise<TableResponseDto> {
    ...
    if (!entity) {
      throw new NotFoundException(
        `Table with ID ${data.Id} and status AVAILABLE not found`,
      );
    }
  ...
  async updateTable(
    ...
    if (!entity) {
      throw new HttpException(
        `Table with ID ${params.Id} not found`,
    ...
    if (!tableResponse) {
      throw new HttpException(
        `Failed to retrieve updated table with ID ${params.Id}`,
  ...
  async deleteTable(data: IdParamDto): Promise<void> {
    ...
    if (!entity) {
      throw new HttpException(
        `Table with ID ${data.Id} not found`,
        HttpStatus.NOT_FOUND,
      );
    }
